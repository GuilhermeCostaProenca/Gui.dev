...
export default function Navbar() {
  return (
    <>
      ...
      <div className="fixed right-6 top-1/2 -translate-y-1/2 z-50 hidden xl:flex">
        <a
      ...
      <motion.header
        ...
        className="nav-pill fixed top-6 left-1/2 z-40 w-[min(94%,82rem)] -translate-x-1/2 rounded-full border border-white/15 backdrop-blur-md"
      >
        <div className="mx-auto flex items-center justify-between px-6 py-3">
          <span className="flex items-center gap-3 text-white">
            <span className="flex h-9 w-9 items-center justify-center rounded-full border border-white/40 text-xs font-semibold">
              G
            </span>
            <span className="text-sm font-semibold tracking-wide">Guilherme</span>
          </span>
          <nav className="hidden lg:flex gap-6 text-white/70 text-[0.7rem] uppercase tracking-[0.3em]">
          ...
          <a
            href="mailto:guilherme@email.com"
            className="hidden sm:inline-flex items-center rounded-full border border-white/30 px-5 py-2 text-xs font-semibold text-white/80 transition hover:border-white hover:text-white"
          >
            Let&apos;s Connect
          </a>
        </div>
